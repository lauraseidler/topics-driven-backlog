...
scp -i id_rsa_${DEPLOYMENT_ENVIRONMENT} -o StrictHostKeyChecking=no variables.production.env ${DEPLOYMENT_USER}@${DEPLOYMENT_HOST}:~
scp -i id_rsa_${DEPLOYMENT_ENVIRONMENT} -o StrictHostKeyChecking=no docker-compose.yml ${DEPLOYMENT_USER}@${DEPLOYMENT_HOST}:~
scp -i id_rsa_${DEPLOYMENT_ENVIRONMENT} -o StrictHostKeyChecking=no docker-compose-production.yml ${DEPLOYMENT_USER}@${DEPLOYMENT_HOST}:~
exit_on_error $?
